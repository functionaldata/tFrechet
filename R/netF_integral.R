...
#' P1_vec <- 0.5 + 0.4*Score[,1] %*% t(phi1(t_out)) + 0.1*Score[,2] %*% t(phi3(t_out)) # edge within first community
#' P2_vec <- 0.5 + 0.3*Score[,3] %*% t(phi2(t_out)) + 0.1*Score[,4] %*% t(phi3(t_out)) # edge within second community
#' 
...
#'   Mat[1:N_net1, 1:N_net1] <- P1_vec[subj, i] # edge within the first communitiy is P1
#'   Mat[(N_net1+1):(N_net1+N_net2), (N_net1+1):(N_net1+N_net2)] <- P2_vec[subj, i] # edge within the second community is P2
#'   diag(Mat) <- 0 #diagonal element is 0
#'   X_mat[i,,] <- Mat
#' }
#' # output the functional principal network(adjacency matrice) of the second eigenfunction
#' netF_integral(Cov_result$phi[,2], t_out, X_mat, 2)
#' @references 
...
netF_integral <- function(phi, t_out, X, U){
  require(fdapace)
  N <- length(phi)
  m <- dim(X)[2]
  if(dim(X)[1] != N){
