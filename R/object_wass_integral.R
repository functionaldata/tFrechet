...
#' @param phi eigenfunction along which we want to project the distribution
#' @param t_out support of phi
#' @param Q A length(t_out) X length(Qout) matrix whose jth row corresponds to the quantile function on grid Qout for the jth timepoint.   
#' @param Qout support of the quantile valued process
#' @return A list of the following:
#' \item{f}{quantile corresponding to the frechet integral of Q along phi}
#' @examples 
#' ### simulation ###
#' n <- 100
#' N <- 50
...
#' }))
#' 
#' score_result <- object_wass_integral(Cov_result$phi[,1], t_out, Q, Qout)
#' score_result$f
#' @references 
...
object_wass_integral <- function(phi, t_out, Q, Qout){
  require(fdapace)
  require(quadprog)
  require(pracma)
  require(frechet)
