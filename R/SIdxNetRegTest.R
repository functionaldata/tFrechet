...
SIdxNetTest <- function(est, b0, xin, Min, nboot = 100, iter = 1000, bw, M, nc = 10, verbose = F){
  ...
  NetBoot_est <- function(est, xin, Min, reps, bw, M, nc, verbose = verbose){
    n = nrow(xin)
    p = length(est)
  ...
  boot_res = NetBoot_est(est = est, xin = xin, Min = Min, reps = nboot, nc = nc, bw = bw, M = M, verbose = verbose)
  ...
  res = list(cov_boot = (M * cov_mat), test_stat = test_stat, pval_chisq = p_val, df = p-1, 
