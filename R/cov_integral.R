...
#' \item{f}{an adjacency matrix which corresponds to the Fréchet integral of X along eig1}
#' @examples
#' set.seed(5)
#' n <- 100
#' N <- 50
...
#' 
#' # simulation based on setting in Dubey, P., & Müller, H. G. (2020). Functional models for time‐varying random objects. 
...
#' P1_cov <- 0.5 + 0.4*Score[,1] %*% t(phi1(t_out)) + 0.1*Score[,2] %*% t(phi3(t_out)) # element within first community 
#' P2_cov <- 0.5 + 0.3*Score[,3] %*% t(phi2(t_out)) + 0.1*Score[,4] %*% t(phi3(t_out)) # element within second community 
...
#' # e.g. subj 2 
#' subj <- 2
#' # X_mat is the network for varying times with X[i,,] is the adjacency matrices for the ith time point
#' X_mat <- array(0, c(N,(N_net1+N_net2), (N_net1+N_net2)))
#' for(i in 1:N){
#'   Mat <- matrix(P12, nrow = (N_net1+N_net2), ncol = (N_net1+N_net2)) # edge between communities is P12
#'   Mat[1:N_net1, 1:N_net1] <- P1_cov[subj, i] # edge within the first communitiy is P1
#'   Mat[(N_net1+1):(N_net1+N_net2), (N_net1+1):(N_net1+N_net2)] <- P2_cov[subj, i] # edge within the second community is P2
...
#' cov_integral(Cov_result$phi[,2], t_out, X_mat)
#' #' @references 
...
cov_integral <- function(phi, t_out, X){
  require(fdapace)
  require(Matrix)
  N <- length(phi)
  m <- dim(X)[2]
